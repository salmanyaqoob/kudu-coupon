...
UserSchema.method({
  ...
  generateAuthToken() {
    ...
    const token = jwt.sign({
      ...
      access,
      exp: Math.floor(Date.now() / 1000) + (60 * 60) }, process.env.JWT_SECRET).toString();
...
UserSchema.statics = {
  ...
  changePassword(_id, currentPassword, newPassword) {
    ...
    return User.findWithPassword({ _id }, currentPassword).then((userData) => {
      const user = userData;
      if (!user) {
        const err = new APIError('No such user exists!', httpStatus.NOT_FOUND);
        return Promise.reject(err);
  ...
  hash: (password, userPassowrd) => new Promise((resolve, reject) => {
    bcrypt.compare(password, userPassowrd, (err, res) => {
      if (res) {
        ...
        const hashErr = new APIError('password not found', httpStatus.NOT_FOUND, true);
        reject(hashErr);
      }
    });
  }),
      ...
      return new Promise((resolve, reject) => {
        bcrypt.compare(password, user.password, (error, res) => {
          if (res) {
            ...
            const userError = new APIError('password not found', httpStatus.NOT_FOUND, true);
            reject(userError);
          }
        });
      });
