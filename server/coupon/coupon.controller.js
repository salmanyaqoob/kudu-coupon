...
async function update(req, res, next) {
  ...
  try {
    ...
    if (!coupon) {
      ...
      // .send({ message: 'Update coupon id not found error' });
      return next(err);
    }
    res.send({ coupon });
  } catch (e) {
 ...
 * @returns {Coupon}
 */
async function getQRCount(req, res) {
  const coupon = req.coupon;
  try {
...
async function createQR(req, res, next) {
  try {
    ...
    if (coupon.total_coupon_limit >= (body.total_qr + couponCount)) {
      ...
      // const err = new APIError('requested QR limit is exceeding with total QR limit try again',
